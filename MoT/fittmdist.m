function tmdist = fittmdist(x,K,varargin)
...
if fitKernel % smooth histogram by fitting a kernel distribution to data and then drawing dummy data from it:
    ...
    x = random(pd,[KernelSize 1]); % draw dummy data
end
...
if strcmpi(Start,'kmeans') || strcmpi(Start,'k-means')
    ...
    for k=1:K
        ...
        fun = @(y)estimateNu(y,u,'init',sum(idx==k),nu(k)); % create function handle
...
for iter=1:MaxIter
    ...
    parfor k = 1:K
        ...
        current_nu = nu(k);
        % CM-2 Step
        fun = @(y)estimateNu(y,u,'est',tau(:,k),current_nu);
...
function f = estimateNu(nu, u,mode,tau,current_nu)

if strcmpi(mode,'init')
    N = tau;
    f = -psi(nu/2) + log(nu/2) + (sum(log(u)-u)/N) + 1 ...
        + psi((1+current_nu)/2) - log((1+current_nu)/2);
